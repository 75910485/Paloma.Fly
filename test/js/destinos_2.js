document.addEventListener("DOMContentLoaded", () => {
    ...
    // FUNCIÓN CLAVE: DEFINE ASIENTOS OCUPADOS DIFERENTES PARA CADA VIAJE
    function getOccupiedSeats(cardIndex) {
        switch (cardIndex) {
            case 0: // Viaje 1 (6:00 AM)
                return { 
                    piso1: ["01", "04", "10"], // Bloquea 01, 04, 10
                    piso2: ["13", "16", "22"] // Bloquea 13, 16, 22
                };
            case 1: // Viaje 2 (1:00 PM)
                return { 
                    piso1: ["02", "05", "07"], // Bloquea 02, 05, 07
                    piso2: ["17", "20", "23"] 
                };
            case 2: // Viaje 3 (11:00 PM)
                return { 
                    piso1: ["03", "06", "08"], // Bloquea 03, 06, 08
                    piso2: ["15", "18", "21", "24"] // Más asientos bloqueados
                };
            default:
                return { piso1: [], piso2: [] };
        }
    ...
    // RENDERIZADO DEL MAPA (AHORA RECIBE EL ÍNDICE DEL VIAJE)
    function renderizarMapa(mapaContenedor, asientosBase, asientosSeleccionados, cardIndex) {
        mapaContenedor.innerHTML = '';
        const occupied = getOccupiedSeats(cardIndex); // Obtiene los ocupados para ESTE viaje
    ...
    // Función de resumen (con destino Ica actualizado)
    ...
    allCards.forEach((card, index) => { // Usamos el 'index' para identificar el viaje
        const mapaContenedor = card.querySelector(".mapa-asientos");
        const estadoInicial = getCardState(card);

        // LÓGICA DEL BOTÓN PAGAR (Se mantiene igual)
        ...
        // Carga inicial: Renderiza el mapa del 1er piso y calcula el resumen.
        // También usa el índice de la tarjeta para cargar los asientos ocupados correctos.
        renderizarMapa(mapaContenedor, asientosPiso1, estadoInicial.piso1, index);
        actualizarResumen(card);
    });
});